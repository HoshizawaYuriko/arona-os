...
| `Rerun` | Boolean | ✅ | `"0"`/`"1"` in the raw response. **Verified broadly** (~980 rows grouped by `(Server, Rateup_character)`, sorted chronologically): each server's own first occurrence of a given rate-up is `"0"` and every later one is `"1"` — held for all but one pairKey. That exception: `SelectPickupFesGacha`'s first-ever tracked occurrence of a combo can already be `"1"` (JP's first `"Wakamo, Hoshino (Swimsuit), Mika"` row) — likely because Archive-type banners only ever feature already-released characters by definition, so `Rerun` there may mean "these characters have run before" rather than "this exact rotation has run before." Now surfaced as a "Rerun" chip on the roadmap, reflecting whichever row is the actual serialized item (the real GL row once confirmed; the JP source row while still only predicted). |
